...
var (
	...
	// Matches the v2 spec pattern: must start and end with alphanumeric;
	// body may contain letters, digits, spaces, underscores, hyphens.
	validJbacNameRegex = regexp.MustCompile(`^[A-Za-z0-9]([A-Za-z0-9 _\-]{0,62}[A-Za-z0-9])?$`)
)
...
func validateJbacName(name string) []string {
	...
	if len(trimmed) > constants.MaxRoleNameLength {
		return []string{fmt.Sprintf("name must be at most %d characters (got %d)", constants.MaxRoleNameLength, len(trimmed))}
	}
	if !validJbacNameRegex.MatchString(trimmed) {
		return []string{"name must start and end with an alphanumeric character and may only contain letters, digits, spaces, underscores, or hyphens"}
	}
	return nil
}
