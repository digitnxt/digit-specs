...
AUTH_ERRORS = [
    ("Missing auth token",  "401", "`code`, `message`", "Layer 3"),
    ("Invalid auth token",  "401", "`code`, `message`", "Layer 3"),
]
MUTATION_ERRORS = [
    ("Missing required fields", "400", "`code`, `message`", "Layer 3"),
    ("Wrong field types",        "400", "`code`, `message`", "Layer 3"),
    ("Invalid URL format",       "400", "`code`, `message`", "Layer 3"),
]
PATH_PARAM_ERROR = ("Key not found", "404", "`code`, `message`", "Layer 3")


def layer_for_status(status):
    return "Layer 2" if int(status) < 400 else "Layer 3"


def rows_for_operation(method, path, operation):
    rows = []
    # Check if endpoint is public (security: [])
    is_public = operation.get("security") == []

    for status, resp_obj in sorted(operation.get("responses", {}).items()):
        code = str(status)
        desc = resp_obj.get("description", "")
        if code.startswith("2") or code == "307":
