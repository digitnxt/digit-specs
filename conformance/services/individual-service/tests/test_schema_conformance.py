...
def test_all_endpoints_conform(case: Case, request, base_url, auth_headers, gateway_headers_spec):
    ...
    Attaches the PreparedRequest to the pytest node so conftest injects
    a cURL command into conformance.html on failure.
    """
    response = case.call(base_url=base_url, headers=auth_headers)

    # Schemathesis uses `requests` internally — grab PreparedRequest for cURL injection
