...
_summary_records = []          # [{test, method, url, status_code, outcome, error}]
_tl = threading.local()        # per-test HTTP log stored in thread-local
...
def pytest_addoption(parser):
    parser.addoption(
        ...
        action="store",
        required=True,
        help="Base URL of the workflow service under test (e.g. http://localhost:8080/workflow/v3)"
    )
    parser.addoption(
        "--api-token",
        action="store",
...
# ── HTTP capture fixture (autouse — wraps every test) ────────────────────────

@pytest.fixture(autouse=True)
def _http_capture(request):
    """Intercepts every requests.Session.send call and logs it for the summary."""
...
# ── Collect results after each test call ─────────────────────────────────────
...
def pytest_runtest_logreport(report):
    ...
    if http_log:
        ...
        # Test made no HTTP calls (e.g. setup error, parametrize skip)
        _summary_records.append({
            "test":        nodeid,
            "method":      "-",
            "url":         "-",
