...
### Cross-field: BusinessService effective date range
...
### Cross-field: Minimum payable amount requires partial payment enabled

**Entities involved:** BusinessServiceRequest (`partialPaymentAllowed`, `minPayableAmount`)  
**Rule:** `minPayableAmount` is only enforced when `partialPaymentAllowed = true`. When `partialPaymentAllowed = false`, any `minPayableAmount` value is ignored. Setting `minPayableAmount` without enabling partial payment has no effect.  
**Violation response:** N/A (ignored if partialPaymentAllowed=false; 400 if amount is invalid when enabled)

---

### Cross-field: Demand period must be ordered
...
### Cross-field: Collected amount cannot exceed total amount

**Entities involved:** Demand (`collectedAmount`, `totalAmount`)  
**Rule:** `collectedAmount` must never exceed `totalAmount`. Any payment or adjustment that would cause `collectedAmount > totalAmount` is rejected with an over-collection error.  
**Violation response:** 422 — `UNPROCESSABLE_ENTITY` (`OVER_COLLECTION_DETECTED`)

---

### Cross-field: Demand total must equal sum of line items
...
### Cross-field: Payer array cardinality
...
### Cross-field: Bill amount paid cannot exceed total
...
### Cross-field: Payment instrument validation by mode
...
### Cross-field: Instrument date age constraint
...
### Cross-field: Payment total must equal sum of payment details
...
### Cross-schema: TaxHead requires active BusinessService
...
### Cross-schema: TaxHead order_number must be unique per service
...
### Cross-schema: LineItem tax head must belong to demand's business service
...
### Cross-schema: No overlapping demand periods for same consumer and service
...
### Cross-schema: Only one active bill per consumer per service
...
### Cross-schema: Bill must be ACTIVE or PARTIALLY_PAID for payment
...
### Cross-schema: Unique payment per bill
...
### Cross-schema: Demand requires active BusinessService
...
### Cross-schema: Bill generation requires at least one qualifying demand
...
### Lifecycle: Demand status transitions
...
### Lifecycle: Only DRAFT and ACTIVE demands are editable
...
### Lifecycle: Bill generation freezes ACTIVE demands
...
### Lifecycle: Bill status transitions
...
### Lifecycle: Payment status transitions

**Entities involved:** Payment (`paymentStatus`, `instrumentStatus`)  
**Rule:** `paymentStatus` transitions: `NEW → DEPOSITED → RECONCILED` (normal path), `NEW/DEPOSITED → CANCELLED` or `DISHONOURED`. `instrumentStatus` transitions depend on payment mode: open states (`APPROVED`, `APPROVAL_PENDING`, `TO_BE_SUBMITTED`, `REMITTED`) and closed states (`REJECTED`, `CANCELLED`, `DISHONOURED`). Closed instrument statuses are terminal.  
**Violation response:** 422 — `UNPROCESSABLE_ENTITY`

---

### Lifecycle: Arrear demands reference their source

**Entities involved:** Demand (`status`), ArrearDemand  
**Rule:** When arrear roll-forward is enabled, a new demand is created with the outstanding amount, and the source demand transitions to `ROLL_FORWARDED`. The new demand's `arrearDemandIds` lists the IDs of the demands that were rolled forward. A `ROLL_FORWARDED` demand cannot be edited or paid directly.  
...
### Cross-module: IDGen required for bill number generation
...
### Cross-module: IDGen required for receipt number generation
...
### Cross-module: Apportion service required for payment distribution
...
### Cross-module: Bulk bill generation via PubSub

**Entities involved:** Bill, PubSub  
**Rule:** `POST /v3/bills/bulk-generate` publishes a message to the `${BULK_BILL_GENERATION_PUBSUB_TOPIC}` topic rather than generating bills synchronously. The event consumer processes the batch. If PubSub is unavailable, the bulk request fails immediately (it cannot fall back to synchronous).  
**Violation response:** 500 — `INTERNAL_SERVER_ERROR`
